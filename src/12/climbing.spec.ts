import { ElMap, getAdjecent, getPosForLetter, invertLetters, parseInput, parsePosition, Pos, serialisePosition, shortestPath } from "./climbing"
...
test("find the fewest steps required from any 'a' to 'E'", () => {
	 ...
	 */
	const elMap = parseInput().map((r) => r.map((c) => c !== 'S' && c !== 'E' ? invertLetters(c) : c))
