...
export function getPosForLetter(elMap: ElMap, letter: string): Pos[] {
  ...
  for (let rowIdx = 0; rowIdx < elMap.length; rowIdx++) {
    for (let colIdx = 0; colIdx < elMap[0].length; colIdx++) {
      if (elMap[rowIdx][colIdx] === letter) {
        positionList.push({ row: rowIdx, col: colIdx });
      }
    }
  }
  return positionList;
...
export function shortestPath(
  ...
  const endPos = end
    ? serialisePosition(getPosForLetter(elMapSE, "E")[0])
    : null;
  const elMap = JSON.parse(JSON.stringify([...elMapSE]));

  const parsePos = parsePosition(startPos);
  elMap[parsePos.row][parsePos.col] = "a";

  if (endPos) {
    const parseEndPos = parsePosition(endPos);
  ...
  while (nodes.length) {
    const smallestNode = nodes.sort((a, b) => b.steps - a.steps).pop();
    if (!smallestNode) {
      console.error("smallest Node undefined!");
      break;
    }
    ...
    if (distances[smallest] !== Infinity) {
      const adjacentPositions = getAdjecent(parsePosition(smallest), elMap);
      for (const adj of adjacentPositions) {
        const serPos = serialisePosition(adj);
...
export function prettyPrint(elMap: ElMap, path: string[], startPos: Pos): void {
  const prettyMap: ElMap = [...Array(elMap.length)].map((r) =>
    [...Array(elMap[0].length)].map((c) => "."),
  );
  ...
  for (const serPos of path) {
    ...
    if (rowDiff < 0) {
      prettyMap[currentPos.row][currentPos.col] = "v";
      currentPos.row = nextStep.row;
      currentPos.col = nextStep.col;
      continue;
    }
    if (rowDiff > 0) {
      prettyMap[currentPos.row][currentPos.col] = "^";
      currentPos.row = nextStep.row;
      currentPos.col = nextStep.col;
      continue;
    }
...
export function getAdjecent(pos: Pos, elMap: ElMap): Pos[] {
  ...
  const validPos: Pos[] = [];
  const targetEl = elMap[pos.row][pos.col];

  for (const a of adjacentPos) {
    if (a.col < 0 || a.col >= elMap[0].length) continue;
    if (a.row < 0 || a.row >= elMap.length) continue;
    const pEl = elMap[a.row][a.col];
    const asciiAdj = pEl.charCodeAt(0);
    const asciiTarget = targetEl.charCodeAt(0);
    if (!asciiAdj || !asciiTarget)
      throw new Error(
        `ascii value undefined! asciiAdj: ${asciiAdj} asciiTarget${asciiTarget}`,
      );
    if (asciiTarget - asciiAdj >= -1) validPos.push(a);
    continue;
  }
